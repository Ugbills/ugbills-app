...
part 'internet_data_controller.g.dart';
...
@riverpod
class InternetDataController extends _$InternetDataController {
  @override
  dynamic build() {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Go.to(SentSuccessfully(
          title: "Internet Data Purchase Successful",
          body: data["msg"]?.toString() ?? "Internet Data purchase completed successfully",
          nextPage: DataTransactionDetails(
            networkLogo: getNetWorkIcon(network),
            dataPlan: dataPlan,
            amount: returnAmount(int.parse(amount.replaceAll("NGN", "").replaceAll(",", ""))),
            transactionID:
            ...
            fee: "10.00",
            note: data["msg"]?.toString() ?? "Internet Data purchase completed",
          ),
        ));
      }
      ...
      if (e.response?.data != null) {
        ...
        errorSnack(context, "Failed to purchase internet data");
      }
